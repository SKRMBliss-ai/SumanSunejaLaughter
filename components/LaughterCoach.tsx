...
export const LaughterCoach: React.FC = () => {
  ...
  const handleFeedback = (rating: 'up' | 'down') => {
    ...
    addPoints(5, "Thanks for Feedback!", 'COACH');
  ...
  const startLiveSession = async () => {
    ...
    if (!apiKey) {
      setError("API Key missing. Live session unavailable.");
  ...
  const stopSession = () => {
    ...
    if (completedSessionType && wasActive) {
        addPoints(completedSessionType === 'LIVE' ? 30 : 20, "Session Completed", 'COACH');
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      console.error(err);
      setError("Please allow microphone access to use the Coach!");
    }
  };
  ...
  const analyzeAudio = async (base64: string, mimeType: string) => {
    try {
      ...
      if (result.score > 0) {
        ...
        addPoints(15, "Laughter Analyzed", 'COACH');
      }
    } catch (err: any) {
      if (err.message === "MISSING_GEMINI_KEY") {
        ...
        setError("Couldn't analyze the laughter. Try again!");
      }
    } finally {
  ...
  const clearHistory = () => {
    if (window.confirm("Are you sure you want to clear your laughter log?")) {
      setHistory([]);
    }
  };
  ...
  return (
    <div className="flex flex-col items-center min-h-[70vh] p-6 pb-32 space-y-8 animate-in fade-in duration-500 relative">
      ...
      {showFeedback && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-pop-in">
           <div className="bg-white dark:bg-slate-800 rounded-[2rem] p-6 w-full max-w-xs shadow-2xl relative border-4 border-[#EDE8F8] dark:border-slate-700 text-center">
               ...
               <h3 className="text-xl font-fredoka font-bold text-gray-700 dark:text-gray-100 mb-2 mt-2">How was it?</h3>
               <p className="text-sm text-[#AABBCC] mb-6">Help Suman AI improve!</p>
      ...
      <div className="w-full max-w-sm space-y-3 animate-fade-in-up">
        ...
        >
           <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                 <h3 className="font-bold text-gray-800 dark:text-gray-100">Start Live Session</h3>
                 <p className="text-xs opacity-70 text-gray-600 dark:text-gray-400">Interactive Conversation</p>
              </div>
           </div>
           {isSessionActive && sessionType === 'LIVE' && (
        ...
        >
           <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                 <h3 className={`font-bold ${isSessionActive && sessionType === 'QUICK' ? 'text-gray-800 dark:text-gray-100' : 'text-white'}`}>Quick 1-Min Laugh</h3>
                 <p className={`text-xs ${isSessionActive && sessionType === 'QUICK' ? 'opacity-70 text-gray-600 dark:text-gray-400' : 'text-white/80'}`}>Guided Energy Boost</p>
              </div>
           </div>
           {isSessionActive && sessionType === 'QUICK' && (
             <div className="flex gap-1 items-end h-4">
      ...
      <div className="text-center space-y-2 animate-pop-in delay-200">
        <h2 className="text-4xl font-fredoka font-bold text-[#C3B8D5] drop-shadow-sm">Laughter Coach</h2>
        <p className="text-[#AABBCC] font-medium">Laugh loudly to get your happiness score!</p>
      </div>
          ...
          <div className="absolute w-44 h-44 rounded-full bg-white dark:bg-slate-800 flex items-center justify-center shadow-lg overflow-hidden p-4">
             {isAnalyzing ? (
                <div className="flex flex-col items-center">
                   ...
                   <span className="text-xs font-bold text-[#AABBCC] animate-pulse">Analyzing Joy...</span>
                </div>
             ) : isMissingKey ? (
               <div className="text-center flex flex-col items-center">
                 <Key className="w-10 h-10 text-red-400 mb-2 animate-bounce" />
                 <span className="text-xs font-bold text-red-500 leading-tight">Gemini API Key Missing!</span>
                 <span className="text-[9px] text-gray-400 mt-1">Falling back to offline mode</span>
               </div>
             ) : scoreData ? (
               <div className="text-center animate-pop-in">
                 ...
                 <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">Joy Score</span>
               </div>
             ) : isSessionActive ? (
                <div className="text-center flex flex-col items-center">
                   ...
                   <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">
                     {sessionType === 'LIVE' ? "AI Listening..." : "Listen & Laugh"}
                   </span>
                </div>
             ) : (
      ...
      <div className="space-y-4 w-full max-w-xs text-center z-10">
        ...
        {isRecording && (
          ...
          >
            <Square fill="currentColor" size={20} /> Stop & Rate
          </button>
        )}
        ...
        {usingOfflineVoice && !error && (
            <div className="text-orange-500 text-xs font-bold bg-orange-50 dark:bg-orange-900/20 p-2 rounded-xl border border-orange-100 dark:border-orange-900/50 flex items-center justify-center gap-2 animate-in fade-in">
                <WifiOff size={12} /> Using Offline Voice (Connection Issue)
            </div>
        )}
        ...
        <div className="w-full max-w-md mt-8 animate-fade-in-up delay-500">
           <div className="flex items-center justify-between mb-4 px-2">
              <h3 className="text-lg font-fredoka font-bold text-gray-600 dark:text-gray-300 flex items-center gap-2">
                 <Calendar size={18} className="text-[#C3B8D5]" /> Laughter Log
              </h3>
              <button 
                ...
                <div key={item.id} className="bg-white dark:bg-slate-800 p-3 rounded-xl flex items-center justify-between shadow-sm border border-transparent hover:border-[#ABCEC9]/30 transition-all hover:scale-[1.01]">
                   <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-md ${
                        item.score >= 80 ? 'bg-[#ABCEC9]' : item.score >= 50 ? 'bg-[#C3B8D5]' : 'bg-[#AABBCC]'
                      }`}>
                         {item.score}
                      </div>
                      <div>
