...
export const Login: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#EDE8F8] flex flex-col items-center justify-center p-4 relative overflow-hidden font-sans">
      
      {/* Fun Animated Background */}
      <div className="absolute top-[-10%] left-[-10%] w-96 h-96 bg-[#C3B8D5] rounded-full mix-blend-multiply filter blur-3xl opacity-40 animate-[bounce-gentle_6s_infinite]"></div>
      <div className="absolute bottom-[-10%] right-[-10%] w-96 h-96 bg-[#ABCEC9] rounded-full mix-blend-multiply filter blur-3xl opacity-40 animate-[bounce-gentle_5s_infinite_reverse]"></div>
        ...
        <div className="absolute inset-0 pointer-events-none z-50 overflow-hidden">
          {[...Array(20)].map((_, i) => (
             <div key={i} className="absolute animate-[fall_3s_ease-out_forwards]" style={{
               ...
               backgroundColor: ['#C3B8D5', '#ABCEC9', '#AABBCC', '#EDE8F8'][Math.floor(Math.random() * 4)],
               width: '10px',
               height: '10px',
               borderRadius: '50%'
             }}></div>
      ...
      <div className="max-w-md w-full relative z-10">
        ...
        <div className="flex flex-col items-center mb-8 relative animate-pop-in">
            <div className="relative group cursor-pointer" onClick={() => setShowConfetti(true)}>
                <div className="w-36 h-36 bg-white rounded-full flex items-center justify-center shadow-2xl border-[6px] border-[#C3B8D5] animate-float overflow-hidden">
                    <img 
                ...
                <div className="absolute -bottom-2 -right-4 bg-[#ABCEC9] text-white text-xs font-black px-4 py-1.5 rounded-full shadow-lg transform rotate-6 group-hover:rotate-12 transition-transform animate-wiggle">
                    Ha Ha Ha!
                </div>
            </div>
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-[2.5rem] shadow-2xl overflow-hidden border border-white/60 animate-fade-in-up delay-200">
            ...
            {step === 'PHONE' ? (
              <div className="space-y-6 animate-fade-in-up">
                ...
                <div className="group relative flex gap-2">
                    ...
                    <div className="relative">
                       <button 
                         ...
                         className="h-full bg-white border-2 border-[#C3B8D5]/30 rounded-2xl px-3 flex items-center gap-1 hover:border-[#ABCEC9] transition-colors"
                         ...
                         <div className="absolute top-full left-0 mt-2 bg-white rounded-xl shadow-xl border border-gray-100 p-1 z-50 w-48 animate-pop-in origin-top-left">
                            {COUNTRIES.map(c => (
                              ...
                              >
                                 ...
                                 {selectedCountry.code === c.code && <Check size={14} className="ml-auto text-[#ABCEC9]" />}
                              </button>
                            ))}
                         </div>
                       )}
                    ...
                    <div className="relative flex-1">
                      <input 
                        ...
                        className="w-full bg-[#EDE8F8]/50 border-2 border-[#C3B8D5]/30 text-gray-800 text-xl font-bold py-5 px-4 rounded-2xl focus:outline-none focus:border-[#ABCEC9] focus:bg-white transition-all placeholder:text-gray-400"
                        onKeyDown={(e) => e.key === 'Enter' && isPhoneValid && handleSendOtp()}
                        required
                      />
                      <div className="absolute right-4 top-1/2 -translate-y-1/2 text-[#C3B8D5]">
                          <Phone size={20} />
                      </div>
                    </div>
                </div>
                ...
                <button 
                  ...
                  className={`w-full bg-gradient-to-r from-[#ABCEC9] to-[#C3B8D5] hover:brightness-95 text-white font-bold py-5 rounded-2xl shadow-xl shadow-[#ABCEC9]/30 transition-all transform hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-3 text-lg ${
                    (isLoading || !isPhoneValid) ? 'opacity-50 cursor-not-allowed scale-100' : ''
                  }`}
                >
                  {isLoading ? <Loader2 className="animate-spin" /> : (
                      <>
                        <span>Send OTP</span>
                        <Zap size={20} className="fill-current text-white/50" />
                      </>
                  )}
                </button>
                ...
                <div className="mt-4 pt-2 text-center">
                    <button 
                      ...
                      disabled={isLoading}
                      className="text-gray-500 hover:text-[#ABCEC9] text-xs font-bold transition-colors inline-flex items-center gap-1 group"
                    >
              ...
              <div className="space-y-6 animate-pop-in">
                <div className="text-center">
                  <div className="inline-flex items-center gap-2 bg-[#ABCEC9]/10 text-[#ABCEC9] px-5 py-2 rounded-full text-xs font-bold mb-6 border border-[#ABCEC9]/20">
                    <span className="w-2 h-2 rounded-full bg-[#ABCEC9] animate-pulse"></span>
                  ...
                  <div className="relative mb-2">
                      <input 
                          ...
                          className="w-full bg-white border-b-4 border-gray-200 focus:border-[#ABCEC9] rounded-xl p-4 text-center text-4xl font-bold tracking-[0.5em] text-gray-800 focus:outline-none transition-all placeholder:text-gray-300 placeholder:tracking-widest"
                          maxLength={6}
                          autoFocus
                ...
                <button 
                  ...
                  disabled={isLoading}
                  className="w-full bg-gradient-to-r from-[#ABCEC9] to-[#8fb8b0] hover:brightness-95 text-white font-bold py-5 rounded-2xl shadow-xl shadow-[#ABCEC9]/30 transition-all transform hover:scale-[1.02] active:scale-95 disabled:opacity-70 flex items-center justify-center gap-3 text-lg"
                >
                  {isLoading ? <Loader2 className="animate-spin" /> : (
                      <>
                ...
                <div className="space-y-4 pt-2">
                    ...
                    className="w-full text-gray-500 text-sm font-bold hover:text-[#ABCEC9] transition-colors underline decoration-dashed underline-offset-4"
                    >
          ...
          {/* Footer Bar */}
          <div className="bg-[#EDE8F8] p-4 flex justify-between items-center text-[10px] text-[#AABBCC] font-bold uppercase tracking-widest border-t border-[#C3B8D5]/20">
